function DirectedAcyclicGraph() {
    ...
    var nodename = function(d) { return d.report["Line"] ? d.report["Line"][0] : ""; };
    var nodename0 = function(d) { return d.report["StationID"] ? d.report["StationID"][0] : ""; };
    ...
    var drawnode = function(d) {
		...
		console.log(color_dict[nodename]);
        // Attach the DOM elements
        var rect = d3.select(this).append("rect").attr("class",nodename);
        var text = d3.select(this).append("text").attr("text-anchor", "middle").attr("x", 0);
        rect.style("stroke",function(d) { return color_dict[d.report["Line"]];}).style("fill",function(d) { return color_dict[d.report["Line"]];});
        text.append("tspan").attr("x", 0).attr("dy", "1em").text(nodename0);
